...
class Hook:
    ...
    def command(self) -> str:
        return f"echo 'Running {self.event}.{self.name} hook...' && {self.path}"
...
def discover_hooks() -> list[Hook]:
    ...
    for path in sorted(HOOKS.iterdir()):
        ...
        if match is None:
            raise SystemExit(
                f"invalid hook filename {path.relative_to(ROOT)}; "
                "expected <event>.<name>.<ext>"
            )
        hooks.append(
            Hook(
...
def sync_hooks() -> int:
    ...
    for section in sorted(desired):
        ...
        has_command = config_value(f"hook.{section}.command") is not None
        if marker is None and has_command:
